...
export const ServerImageLinks: React.FC<{ server: MinecraftServer }> = ({ server }) => {
    return (
        <div className="flex flex-col space-y-1">
            ...
            <textarea
                className="form-input mt-1 block w-full border-solid border-2 rounded-md resize-none"
                defaultValue={`[${server.imageUrl} https://${server.host}]`}
            />
            <p>Markdown</p>
            <textarea
                className="form-input mt-1 block w-full border-solid border-2 rounded-md resize-none"
                defaultValue={`[![${server.host}](${server.imageUrl})](https://${server.host})`}
            />
            <p>HTML</p>
            <textarea
                className="form-input mt-1 block w-full border-solid border-2 rounded-md resize-none"
                defaultValue={`<a href="https://${server.host}"><img src="${server.imageUrl}"></a>`}
            />
        </div>
